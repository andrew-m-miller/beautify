// MT_BeautyFS - pass 4 of 6
//
// Horizontal half of the base blur.  This runs on the already mid-blurred
// image rather than on the front, so the sigma needed to reach the requested
// base radius is sqrt(base^2 - mid^2): two gaussians in series add in
// quadrature.  Alpha is passed straight through, the composite only needs the
// procedural texture at the detail radius.
...
void main() {
	...
	vec4 centre = texture2D(adsk_results_pass3, xy / res);
	...
	for (float i = stride; i <= support; i += stride) {
		...
		vec3 s0 = texture2D(adsk_results_pass3, (xy + off) / res).rgb;
		vec3 s1 = texture2D(adsk_results_pass3, (xy - off) / res).rgb;
