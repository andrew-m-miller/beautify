// MT_BeautyFS - pass 3 of 6
//
// Vertical half of the detail-radius blur.  The result is the mid-frequency
// image: RGB holds the skin with pore-scale texture removed, alpha holds the
// matching low-passed copy of the procedural texture.
...
void main() {
	...
	for (float i = stride; i <= support; i += stride) {
		...
		vec4 s0 = texture2D(adsk_results_pass2, (xy + off) / res);
		vec4 s1 = texture2D(adsk_results_pass2, (xy - off) / res);
